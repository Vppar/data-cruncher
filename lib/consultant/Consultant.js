...
function Consultant(uuid, name, mkCode, cep, address, cpf, bank, agency, account, email) {
  ...
  if (arguments.length !== Consultant.length) {
    if (arguments.length === 1 && _.isObject(arguments[0])) {
      ...
      throw Error('Consultant must be initialized with uuid, name, mkCode, cep, address, cpf, bank, agency, account, email');
    }
  } else {
    this.uuid = uuid;
    this.name = name;
