...
function ConsultantService(consultantInterface) {
  ...
  this.isValid = function(consultant) {
    ...
    invalidProperty.name = _.isUndefined(consultant.name);
  ...
  this.list = function() {
    var result = null;
    try {
      result = consultantInterface.list();
    } catch (err) {
      console.log('ConsultantInterface.list: Unable to recover the list of Consultant. Err=' + err);
    }
    return result;
  };
  ...
  this.read = function(uuid) {
    var result = null;
    try {
      result = consultantInterface.read(uuid);
    } catch (err) {
      console.log('ConsultantInterface.read: Unable to find a consultant with id=\'' + uuid + '. Err=' + err);
    }
    return result;
  };
  ...
  this.create = function(consultant) {
    ...
    if (hasErrors.length === 0) {
      result = consultantInterface.create(consultant);
    } else {
  ...
  this.update = function(consultant) {
    ...
    if (result.length === 0) {
      consultant = new Consultant(consultant);
      var promise = consultantInterface.update(consultant);
