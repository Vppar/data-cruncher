...
function CheckService(checkInterface) {
...
//
//  _.extend(handlers, checkHandler.handlers);
  ...
  this.addCheck = function(check) {
    ...
    if (hasErrors.length === 0) {
      ...
      result = checkInterface.add(checkEntry);
    } else {
  ...
  this.changeState = function(uuid, newState) {
    ...
    if (result) {
      return checkInterface.changeState(uuid, newState);
    } else {
      return deferred.reject('Invalid transition from "' + check.state + '" to "' + newState + '"');
    }
  };
  ...
  this.read = function(uuid) {
    var result = null;
    try {
      result = checkInterface.read(uuid);
    } catch (err) {
     console.log('CheckService.read: Unable to find a check with the uuid=' + uuid + '. ' + 'Err=' + err);
    }
    return result;
  };
  ...
  this.isValid = function(item) {
    ...
    invalidProperty.bank = _.isUndefined(item.bank);
    invalidProperty.agency = _.isUndefined(item.agency);
    invalidProperty.account = _.isUndefined(item.account);
    invalidProperty.number = _.isUndefined(item.number);
    invalidProperty.duedate = _.isUndefined(item.duedate) && item.duedate <= now;
    invalidProperty.amount = _.isUndefined(item.amount) && _.isNumber(item.amount) && item.amount > 0;
