...
function PurchaseOrderInterface(journalKeeper, purchaseOrderHandler) {
  ...
  this.add = function (purchase) {
    ...
    purchaseObj.status = purchaseOrderHandler.STATUS_STASHED;
  ...
  this.update = function update(purchaseOrder) {
    ...
    if (!recoveredPurchaseOrder) {
      throw Error('Unable to find an PurchaseOrder with uuid=\'' + purchaseOrder.uuid + '\'');
    }
  ...
  this.changeStatus = function changeStatus(uuid, statusName) {
    var recoveredPurchaseOrder = ArrayUtils.find(purchaseOrderHandler.purchases, 'uuid', uuid);
  ...
  this.cancel = function cancel(uuid) {
    ...
    var cancelEv = {
      uuid: purchase.uuid,
      status: purchaseOrderHandler.STATUS_CANCELED,
      updated: now,
  ...
  this.receiveProduct = function (uuid, productId, nfeNumber, extNumber, qty) {
    ...
    if (!item) {
      throw Error('Unable to find in PurchaseOrder uuid=\'' + uuid + '\'' + ' an item with id=\'' + productId + '\'');
    }
    ...
    var receiveEv = {
      uuid: purchase.uuid,
      productId: productId,
      status: purchaseOrderHandler.STATUS_PARC_REC,
      updated: now,
      nfeNumber: nfeNumber,
      extNumber: extNumber,
      received: now,
    ...
    return this.journalize(receiveEv);
  };
}
