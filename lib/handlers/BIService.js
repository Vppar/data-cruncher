...
function BIService(biRef, intervals) {
    ...
    this.charts = function () {
        ...
        for (var k = histogram.bands.length - 1; k >= 0; k--) {
            if (histogram.bands[k].snapshot === 0 && k !== 0 ) {
                delete histogram.bands[k].snapshot;
                delete cumulative.bands[k].snapshot;
            } else {
                histogram.current = k;
                cumulative.current = k;
                termometer.snapshot = cumulative.bands[k].snapshot;
                termometer.goal = cumulative.bands[k].goal;
                termometer.percent = Math.round(termometer.snapshot / termometer.goal * 100) + '%';
                break;
            }
        }
