...
function MisplacedService() {
    ...
    this.distributeSpecificDiscount = function (total, discount, items) {
        ...
        for (var idy in items) {
            var forItem = items[idy];

            var forItemTotal = forItem.qty * (forItem.price || forItem.cost || forItem.amount);

            var itemShare = forItemTotal / total;
            var itemDiscount = Math.round(100 * discount * itemShare) / 100;
            var isLastItem = parseInt(idy) === (items.length - 1);
            var currentTotalDiscount = Math.round(100 * (currTotalDiscount + itemDiscount)) / 100;
            var discountDoesNotMatch = currentTotalDiscount !== discount;

            forItem.specificDiscount = itemDiscount;
            ...
            if (isLastItem && discountDoesNotMatch) {
                var discountDiff = discount - currentTotalDiscount;
    ...
    this.distributeOrderDiscount = function (discount, items) {
        ...
        for (var idx in items) {
            ...
            itemTotal = item.qty * (item.price || item.cost || item.amount);
        ...
        for (var y in items) {
            ...
            var itemDiscount = Math.round(100 * discount * itemShare) / 100;
            var isLastItem = parseInt(y) === (items.length - 1);
            var currentTotalDiscount = Math.round(100 * (currTotalDiscount + itemDiscount)) / 100;
            var discountDoesNotMatch = currentTotalDiscount !== discount;

            items[y].orderDiscount = itemDiscount;
    ...
    this.discount = function () {
        ...
        function distributeByWeight(items, value, getTotal) {
            ...
            for (var idx in items) {
                ...
                var tempValue = _normalizeDecimals(tmpValue + itemValue);

                weightedValues.push(itemValue);

                var isLastItem = parseInt(idx) === items.length - 1;
                var valueDoesNotMatch = tempValue !== value;
                ...
                if (isLastItem && valueDoesNotMatch) {
                    var valuesDiff = value - tempValue;
...
module.exports = new MisplacedService();
